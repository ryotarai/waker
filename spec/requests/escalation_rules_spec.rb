...
RSpec.describe "EscalationRules", :type => :request do
  ...
  describe "GET /api/v1/escalation_rules" do
    it "returns escalation rules" do
      escalation_rule = create(:escalation_rule)
      get api_escalation_rules_path, default_params
      expect(response.body).to be_json_as([{
        ...
        'url' => api_escalation_rule_url(escalation_rule, format: :json),
      }])
      expect(response.status).to be(200)
    end
  end

  describe "GET /api/v1/escalation_rules/1" do
  ...
  describe "POST /api/v1/escalation_rules" do
    it "creates an escalation rule" do
      ...
      post api_escalation_rules_path, default_params.merge(escalation_rule: attributes)
